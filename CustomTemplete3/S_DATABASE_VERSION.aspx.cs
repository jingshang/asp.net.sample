...
public partial class kurihara_CustomTemplete3_S_DATABASE_VERSION : System.Web.UI.Page
{
    ...
    protected void UPDATE_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            try
            {
                command.CommandText = @"update S_DATABASE_VERSION set  F_VERSION=@F_VERSION,F_CREATE_DATE=@create_date where F_ID=@id";
                command.Parameters.Add(new SqlParameter("@id", ID.Text));
                command.Parameters.Add(new SqlParameter("@F_VERSION", VERSION.Text));
                command.Parameters.Add(new SqlParameter("@create_date", CREATE_DATE.Text));
                // 親テーブルを挿入するSQLの実行
