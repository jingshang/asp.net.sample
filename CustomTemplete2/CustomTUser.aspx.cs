...
public partial class kurihara_CustomTemplete2_CustomTUser : System.Web.UI.Page
{
...
FROM T_USER ", con);
                ...
                this.T_USER.DataSource = table1;
                this.T_USER.DataKeyNames = new string[11] { "F_ID", "F_LAST_NAME", "F_LAST_NAME_KANA", "F_CREATE_USER_ID", "F_CREATE_DATE","F_FIRST_NAME", "F_FIRST_NAME_KANA","F_EMAIL","F_PASS","F_UPDATE_DATE","F_UPDATE_USER_ID" };
                this.T_USER.DataBind();
                sdr1.Close();
            }
            catch
            {
...

    {
        ...
        var f_id = T_USER.DataKeys[row.RowIndex].Values["F_ID"].ToString();
        var f_last_name = T_USER.DataKeys[row.RowIndex].Values["F_LAST_NAME"].ToString();
        var f_first_name = T_USER.DataKeys[row.RowIndex].Values["F_FIRST_NAME"].ToString();
        var f_last_name_kana= T_USER.DataKeys[row.RowIndex].Values["F_LAST_NAME_KANA"].ToString();
        var f_first_name_kana = T_USER.DataKeys[row.RowIndex].Values["F_FIRST_NAME_KANA"].ToString();
        var f_email = T_USER.DataKeys[row.RowIndex].Values["F_EMAIL"].ToString();
        var f_pass = T_USER.DataKeys[row.RowIndex].Values["F_PASS"].ToString();
        var f_create_user_id = T_USER.DataKeys[row.RowIndex].Values["F_CREATE_USER_ID"].ToString();
        var f_update_user_id = T_USER.DataKeys[row.RowIndex].Values["F_UPDATE_USER_ID"].ToString();
        var f_update_date = T_USER.DataKeys[row.RowIndex].Values["F_UPDATE_DATE"].ToString();
        var f_create_date = T_USER.DataKeys[row.RowIndex].Values["F_CREATE_DATE"].ToString();
